...
class Tnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.max_pool(x).view(bs, -1) # modified
        # pass through MLP
        x = self.nonlinear(x)
        x = self.dropout(x)
        x = self.fc(x)
...
class PointNetBackbone(nn.Module):
    ...
    def forward(self, x):
        ...
        x, critical_indices = self.max_pool(x)
...
class PointNetClassHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.local_feats = False
        # get the backbone (only need global features for classification)
        self.backbone = PointNetBackbone(config)
        # MLP for classification
        self.nonlinear = nn.Sequential(
            NonlinearBlock(self.dim_global_feats, 256),
            NonlinearBlock(256, 128)
        )
        self.dropout = nn.Dropout(p=self.dropout_ratio)
